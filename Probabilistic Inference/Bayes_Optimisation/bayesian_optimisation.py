...
class BayesianOptimisation(object):
    ...
    def _bayesian_optimisation_step(self,
        ...
        """
        # Query true objective at the previous argmax point
        obj_fn_value = self._objective_function.evaluate(arg_max_acquisition_function)
        # Update GP using the new data
        self._gaussian_process.add_data_point(arg_max_acquisition_function, obj_fn_value)
        # Optimise the GP
        optimised_parameters = self._gaussian_process.optimise_parameters()
        # Select the new argmax point by optimising the acquisition function
        next_arg_max = self._acquisition_function.compute_arg_max(self._gaussian_process, self._objective_function)
